...
from lawsafrica_cli.client import ContentAPIClient, ContentAPIError, normalize_frbr_uri


class ContentAPIClientTestCase(unittest.TestCase):
    ...
    def test_normalizes_and_encodes_frbr_uri(self):
        ...
        with self.assertRaisesRegex(ContentAPIError, "must begin"):
    ...
    def test_non_json_and_api_errors_are_safe(self):
        with self.make_client(lambda request: httpx.Response(200, text="not json")) as client:
            with self.assertRaisesRegex(ContentAPIError, "not valid JSON"):
                client.get_json("places")

        with self.make_client(lambda request: httpx.Response(403, json={"detail": "Forbidden"})) as client:
            with self.assertRaisesRegex(ContentAPIError, r"403.*Forbidden") as error:
                client.get_json("places")
        self.assertNotIn("test-token", str(error.exception))

    def test_missing_api_key_is_rejected(self):
        with self.assertRaisesRegex(ContentAPIError, "LAWSAFRICA_API_KEY"):
            ContentAPIClient("")
